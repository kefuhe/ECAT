"""
Fixed-geometry BLSE slip-inversion template.

Edit the data paths, shared projection, fault geometry, YAML filenames and
BLSE penalty weight in their existing sections. The script intentionally keeps a
...
The compact ``--mode loop`` entry uses the same canonical penalty scanner as
the dedicated L-curve template. New publication-oriented smoothing searches
should start from test_BLSE_L_Curve.py.
"""
...
from eqtools.csiExtend.blse_diagnostics import plot_blse_lcurve_summary


if __name__ == '__main__':
    # ========================= Runtime options ==========================
    parser = argparse.ArgumentParser(
        description='Run a fixed-geometry BLSE inversion or smoothing scan.'
    )
    parser.add_argument('--mode', choices=['single', 'loop'], default='single',
                       help='Execution mode: single run or penalty-weight scan (default: single)')
    ...
    inversion = BoundLSEMultiFaultsInversion(
        ...
        config='default_config_BLSE.yml', bounds_config='bounds_config.yml',
    )
    ...
    if args.mode == 'single':
        ...
        summary.to_csv(output_dir / 'blse_l_curve.csv', index=False)
        fit_stats.to_csv(
            output_dir / 'blse_l_curve_fit_statistics.csv', index=False,
        )
        fig, _ = plot_blse_lcurve_summary(
            summary, preferred_penalty_weight=10.0,
        )
        fig.savefig(output_dir / 'blse_l_curve.png', dpi=300)
        plt.close(fig)
    ...
    if args.mode == 'single':
        ...
        # data using the parsed vertical/poly settings. BLSE has one final model,
