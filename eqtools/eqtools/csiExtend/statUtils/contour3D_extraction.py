...
class Contour3DExtraction(SourceInv):
    ...
    def calContour3d(self, subpatch_st=None, subpatch_ed=None, ZUp=False, write2file=True, 
                     XYZinds=[0, 2, 1], srange=[0, 4.0], levels=3, outputfile=None, tolerance=1e-6):
        """
        Calculate 3D contours for the fault slip distribution.
    
        Parameters:
        - subpatch_st (int): Starting index of the subpatch (default is None).
        - subpatch_ed (int): Ending index of the subpatch (default is None).
        - ZUp (bool): Whether to set Z-axis upwards (default is False).
        - write2file (bool): Whether to write the results to a file (default is True).
        - XYZinds (list): Order of XYZ indices for the calculation (default is [0, 2, 1]).
        - srange (list): Range of the slip values (default is [0, 4.0]).
        - levels (int or array-like): Number and positions of the contour lines/regions (default is 3).
        - outputfile (str): Name of the output file (default is None).
        - tolerance (float): Tolerance for determining if a point is on the boundary (default is 1e-6).
    
        Returns:
        - line3d (list): List of 3D contour lines.
        - lec (list): List of contour levels.
        """
        ...
        # Calculate distance matrix between triangle points and triangle centers; unit is km
        ...
        # XYZinds indicates the order of XYZ for interpolation
        line3d, lec = extract_cline(vertices, topo, slip_norm, invdist=invdist, 
                                    triinds=list(range(subtris_node[0], subtris_node[1])), 
                                    XYZinds=XYZinds, srange=srange, levels=levels, tolerance=tolerance)
    
        # Save the contour lines of the source fault
        self.line3d_in_source = line3d
        # Save the contour levels of the source fault
        self.lec = lec
    ...
    def plot_contour3d(self, data='receiver', ZUp=False, style=['science', 'no-latex'], 
                       ...
                       azimuth=None, elevation=45, shape=(1, 1, 0.45), show_grid=False):
        """
        ...
        - slip_unit (str): Unit of slip. Default is 'm'.
        - savefig (bool): If True, saves the figure. Default is True.
        - figname (str): Name of the saved figure file. Default is 'contour3d.png'.
        - dpi (int): Dots per inch for the saved figure. Default is 600.
        - azimuth (float): Azimuth angle for the 3D plot. Default is None.
        - elevation (float): Elevation angle for the 3D plot. Default is 45.
        - shape (tuple): Shape of the 3D plot. Default is (1, 1, 0.45).
        - show_grid (bool): If True, shows the grid on the plot. Default is False.
        ...
        with sci_plot_style(style=style, fontsize=fontsize, figsize=figsize):
            ...
            for iline, ic in zip(line3d, lec):
                ...
                if label not in plotted_labels:
                    ...
                    ax.plot3D(lon, lat, z, label=label, color=color, zorder=2)
                    plotted_labels[label] = color
                else:
                    ax.plot3D(lon, lat, z, color=plotted_labels[label], zorder=2)
            ...
            # 优化3D绘图
            optimize_3d_plot(ax, zratio=None, shape=shape, zaxis_position='bottom-left', 
                             show_grid=show_grid, grid_color='#bebebe', background_color='white', axis_color=None, grid_which='major')
...
def plot_contourinsurf(x, y, z, t, levels=3, dtype='tri', topo=None, N=None,
                        intptype='linear', intpidx=None, XYZinds=[0, 1, 2], tolerance=1e-6):
    '''
    ...
    Kwargs   :
        ...
        * tolerance  : float, optional; tolerance for determining if a point is on the boundary
    ...
    boundary_polygon = Polygon(points[hull.vertices])

    # # Plot the boundary polygon for visualization
    # plt.figure()
    # plt.plot(points[:, 0], points[:, 1], 'o', label='Points')
    # for simplex in hull.simplices:
    #     plt.plot(points[simplex, 0], points[simplex, 1], 'k-')
    # plt.plot(*boundary_polygon.exterior.xy, 'r--', label='Boundary Polygon')
    # plt.legend()
    # plt.title('Boundary Polygon and Points')
    # plt.show()
    
    # Function to check if a point is on the boundary within a tolerance
    def is_boundary(pt):
        point = Point(pt)
        return boundary_polygon.exterior.distance(point) < tolerance
    
    # Corresponding to different levels of line segment list, isoline extraction
    allsegs = conts.allsegs
    levels = conts.levels[1:-1] if len(conts.levels) == len(allsegs) + 2 else conts.levels
    intpsegs = []
    for i in range(len(levels)):
        ...
        if i < len(allsegs) and len(allsegs[i]) > 0:
            ...
            for segi in segis:
                cx = segi[:, 0]
                cy = segi[:, 1]
                # Split segments based on boundary points
                segments = []
                current_segment = []
                for j in range(len(cx)):
                    current_segment.append([cx[j], cy[j]])
                    if len(current_segment) == 1:
                        continue
                    if j == len(cx) - 1 or (is_boundary(current_segment[0]) and is_boundary(current_segment[-1])) or (np.allclose(current_segment[0], current_segment[-1], atol=tolerance)):
                        segments.append(np.array(current_segment))
                        current_segment = []
                for segment in segments:
                    cz = griddata(xyz[intpidx, :2], xyz[intpidx, 2], (segment[:, 0], segment[:, 1]), method=intptype)
                    cxyz = np.hstack((segment[:, 0][:, None], segment[:, 1][:, None], cz[:, None]))
    ...
    # All Done.
    # lec is the value of the contour line
    return line3d, lec
   

def extract_cline(XYZ, cotopo, mag_slip, invdist=None, triinds=None, XYZinds=[0, 2, 1], srange=[-1, 1.], levels=3, genColorbar=False, tolerance=1e-6):
    '''
    Step     :
    ...
    Kwargs   :
        ...
        * levels     : int or array-like, optional; levels of contours
            ...
            If an int n, use MaxNLocator, which tries to automatically choose no more than n+1 
                "nice" contour levels between between minimum and maximum numeric values of Z.
            If array-like, draw contour lines at the specified levels. The values must be in increasing order.
        * genColorbar  : Whether to generate the color bar
        * tolerance    : float, optional; tolerance for determining if a point is on the boundary
    ...
    if invdist is None:
        ...
        # Calculate the distance between the nodes and the center of the sub-elements, unit is km
    ...
    # Interpolate the 3-th axis coordinate of the nodes to the center of the sub-elements
    ...
    line3d, lec = plot_contourinsurf(XYZ[:, indx], XYZ[:, indy], 
                                         ...
                                         XYZinds=XYZinds, intpidx=idxs, tolerance=tolerance)
