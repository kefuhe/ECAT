...
class multifaultsolve_boundLSE(multifaultsolve):
    ...
    def calculate_moment_magnitude(self, faults=None, mu=3.e10, slip_factor=1.0, mode='hankel'):
        ...
        Parameters:
        - faults: List of fault objects or fault names to analyze (default is None, uses self.faults)
        - mu (float): Shear modulus (default is 3.e10 Pa)
        - slip_factor (float): Factor to scale the slip (default is 1.0)
        - mode (str): Calculation mode, 'csi' or 'hankel' (default is 'hankel')
        
        Returns:
        - dict: Dictionary containing moment and magnitude information
        """
        # Handle faults parameter
        if faults is None:
            target_faults = self.faults
        elif isinstance(faults, list):
            if len(faults) > 0:
                if isinstance(faults[0], str):
                    # List of fault names
                    fault_dict = {fault.name: fault for fault in self.faults}
                    target_faults = []
                    for fault_name in faults:
                        if fault_name in fault_dict:
                            target_faults.append(fault_dict[fault_name])
                        else:
                            print(f"Warning: Fault '{fault_name}' not found in self.faults")
                else:
            ...
            # print("Warning: Invalid faults parameter, using self.faults")
            target_faults = self.faults
    ...
    def _calculate_moment_magnitude_hankel(self, target_faults, mu=3.e10, slip_factor=1.0):
        """
        Calculate moment magnitude using Hankel formula (2/3 * (log10(Mo) - 9.1)).
        """
        import numpy as np
    ...
    def print_moment_magnitude(self, faults=None, mu=3.e10, slip_factor=1.0, mode='hankel'):
        """
        Print seismic moment and moment magnitude information.
        ...
        results = self.calculate_moment_magnitude(faults=faults, mu=mu, slip_factor=slip_factor, mode=mode)
        ...
        if mode == 'csi':
            ...
            # Print individual fault information
    ...
    def _print_fault_statistics(self, fault_groups=None):
        """Print fault statistics in a beautiful table format."""
        from tabulate import tabulate
        import numpy as np
        ...
        for ifault in self.faults:
            ...
            # Calculate length using cumdistance method
            try:
        ...
        print("\nFault Geometry:")
        print(tabulate(geometry_table_data, headers=geometry_headers, tablefmt='grid', stralign='left'))
        
        # Fault slip statistics table (only min, mean, max - no std)
        slip_table_data = []
        for ifault in self.faults:
            if hasattr(ifault, 'slip') and ifault.slip is not None:
                ...
                if ifault.slip.shape[1] >= 2:
                    ...
                    # Calculate statistics (min, mean, max only)
                    ss_stats = {
                    ...
                    slip_table_data.append([
                        '',
                        'Total slip',
                        f"{total_stats['mean']:.4f}",
                        f"{total_stats['max']:.4f}",
                        f"{total_stats['min']:.4f}"
                    ])
                else:
                    # Only one slip component
                    slip = ifault.slip[:, 0]
                    slip_stats = {
                        'mean': np.mean(slip),
                        'max': np.max(slip),
        ...
        print(tabulate(slip_table_data, headers=slip_headers, tablefmt='grid', stralign='left'))
        
        # Calculate and print seismic moment for each fault and fault groups
        moment_table_data = []
        
        # Use default fault groups if not provided
        if fault_groups is None:
            if hasattr(self, 'config') and hasattr(self.config, 'alpha') and 'faults' in self.config.alpha:
                fault_groups = self.config.alpha['faults']
            else:
                # Fallback: treat each fault as its own group
                fault_groups = [[fault.name] for fault in self.faults]
        
        # Individual fault moments
        fault_moments = {}
        for ifault in self.faults:
            if hasattr(ifault, 'slip') and ifault.slip is not None:
                # Calculate total slip magnitude
        ...
        moment_headers = ['Fault Name/Group', 'Moment (N·m)', 'Magnitude', 'Mean Slip (m)', 'Max Slip (m)']
        print("\nSeismic Moment and Magnitude:")
        print(tabulate(moment_table_data, headers=moment_headers, tablefmt='grid', stralign='left'))
