...
class planarfault(RectangularPatches):
    ...
    def buildPatches(self, lon, lat, dep, strike, dip, f_length, f_width, n_strike, n_dip, verbose=True, discretize=True):
        ...
        # Pre-calculate the patches
        p_width_cos_dip_rad = p_width * np.cos(dip_rad)
        p_width_sin_dip_rad = p_width * np.sin(dip_rad)
        sin_dipdirection_rad = np.sin(dipdirection_rad)
        cos_dipdirection_rad = np.cos(dipdirection_rad)
        ...
        for i in range(self.numz):
            ...
            xb = xt + p_width_cos_dip_rad*sin_dipdirection_rad # p_width * np.cos(dip_rad) * np.sin(dipdirection_rad)
            yb = yt + p_width_cos_dip_rad*cos_dipdirection_rad # p_width * np.cos(dip_rad) * np.cos(dipdirection_rad)
            lonb, latb = self.xy2ll(xb, yb)
            zb = zt + p_width_sin_dip_rad # p_width*np.sin(dip_rad)
            # fill D
            D.append(zb.max())
