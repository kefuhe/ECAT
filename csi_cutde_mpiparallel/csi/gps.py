...
class gps(SourceInv):
    ...
    def read_from_en_gmt(self, velfile, factor=1., minerr=1., header=0, rho=False, dep=False):
        '''
        Reading velocities from a en file formatted as:
    ...
    def read_from_enu_gmt(self, velfile, factor=1., minerr=1., header=0, checkNaNs=True, rho=False, dep=False):
        '''
        Reading velocities from a enu file formatted as

        +-----+-----+-------+-------+-------+-------+-------+-------+-------------+
        | Lon | Lat | e_vel | n_vel | u_vel | e_err | n_err | u_err | StationName |
        +=====+=====+=======+=======+=======+=======+=======+=======+=============+
        |     |     |       |       |       |       |       |       |             |
        |     |     |       |       |       |       |       |       |             |
        +-----+-----+-------+-------+-------+-------+-------+-------+-------------+
        Lon Lat [dep] e n u se sn su [sen seu snu] sta
        ...
        for i in range(header,len(Vel)):
            ...
            if 'nan' not in A or not checkNaNs:
                ...
                east = float(A[2 + disp_ind_offset])
                north = float(A[3 + disp_ind_offset])
                up = float(A[4 + disp_ind_offset])
                self.vel_enu.append([east, north, up])

                east = float(A[5 + disp_ind_offset])
                north = float(A[6 + disp_ind_offset])
                up = float(A[7 + disp_ind_offset])
                if east == 0.:
                    east = minerr
                if north == 0.:
                    north = minerr
    ...
    def deletestation(self, station):
        ...
        if u.size > 0:
            ...
            if self.vel_enu is not None and self.vel_enu.ndim > 0:
                self.vel_enu = np.delete(self.vel_enu, u, axis=0)
            if self.err_enu is not None and self.err_enu.ndim > 0:
    ...
    def getNumberOfTransformParameters(self, transformation):
        ...
        if transformation.__class__ in (str, type(None)): # Changed by kfhe at 01/16/2023 (str,) to (str, type(None))
        ...
        for itransformation in transformation:
            ...
            elif itransformation == 'strainnorotation':
                tmpNpo = 5
            # Strain Only (Strain)
            elif itransformation == 'strainonly':
                tmpNpo = 3
            # Strain without translation (Strain + Rotation)
            elif itransformation == 'strainnotranslation':
                tmpNpo = 4
            # Translation
            elif itransformation == 'translation':
                tmpNpo = 2
            # Translation and Rotation
            elif itransformation == 'translationrotation':
                tmpNpo = 3
            ...
            # If verticals
            if not np.isnan(self.vel_enu[:,2]).any() and itransformation not in ('full', 'eulerrotation', 'internalstrain'):
                tmpNpo += 1
    ...
    def getTransformEstimator(self, transformation, computeNormFact=True, computeIntStrainNormFact=True):
        ...
        Args:
            * transformation : String. Can be 'strain', 'full', 'strainnorotation', 'strainnotranslation', 'strainonly', 
            'translation' or 'translationrotation' or 'eulerrotation' or 'internalstrain'
        ...
        for itransformation in transformation:
            # Helmert Transform
            if itransformation == 'full':
                tmporb = self.getHelmertMatrix(components=self.obs_per_station)
            # Strain + Rotation + Translation
            elif itransformation == 'strain':
                tmporb = self.get2DstrainEst(computeNormFact=computeNormFact)
            # Strain + Translation
            elif itransformation == 'strainnorotation':
                tmporb = self.get2DstrainEst(rotation=False, computeNormFact=computeNormFact)
            # Strain
            elif itransformation == 'strainonly':
                tmporb = self.get2DstrainEst(rotation=False, translation=False, computeNormFact=computeNormFact)
            # Strain + Rotation
            elif itransformation == 'strainnotranslation':
                tmporb = self.get2DstrainEst(translation=False, computeNormFact=computeNormFact)
            # Translation
            elif itransformation == 'translation':
                tmporb = self.get2DstrainEst(strain=False, rotation=False, computeNormFact=computeNormFact)
            # Translation and Rotation
            elif itransformation == 'translationrotation': 
                tmporb = self.get2DstrainEst(strain=False, computeNormFact=computeNormFact)
            elif itransformation == 'eulerrotation':
                tmporb = self.getEulerMatrix(components=self.obs_per_station)
            elif itransformation == 'internalstrain':
                tmporb = self.getInternalStrain(components=self.obs_per_station, computeIntStrainNormFact=computeIntStrainNormFact)
            # Unknown case
            else:
    ...
    # ---------------------------------------------------------------------- 
    def computeTransformation(self, fault, verbose=False, custom=False, computeNormFact=True, computeIntStrainNormFact=True):
        '''
        ...
        if transformation.__class__ in (str,):
            transformation = [transformation]

        # make the array
        self.transformation = np.zeros(self.vel_enu.shape)
        # start point and end point of subtransform 
        st = 0
        se = 0
        for itransformation in transformation:
            ...
            # modified by kfhe, 10/10/2021
            # Get the corresponding values
            # vec = fault.polysol[self.name][transformation]
            if fault.type == 'transformation':
                vec = fault.polysol[self.name][itransformation]
            else:
                vec = fault.polysol[self.name]

            itn = self.getNumberOfTransformParameters(itransformation)
            se += itn
            vec = vec[st:se]
            # Used to anchoring the location of Polysol for itransform
            setattr(self, '{}Index'.format(itransformation), [st, se])
            st += itn
    ...
    def removeTransformation(self, fault, custom=False, verbose=False, computeNormFact=True, computeIntStrainNormFact=True):
        '''
        Removes the transformation that is stored in a fault.
        '''

        # Compute the transformation
        self.computeTransformation(fault, custom=custom, verbose=False, computeNormFact=computeNormFact, computeIntStrainNormFact=computeIntStrainNormFact)
    ...
    def compute2Dstrain(self, fault, write2file=False, verbose=False, computeNormFact=True):
        '''
        ...
        # Compute the transformation
        self.computeTransformation(fault, computeNormFact=computeNormFact)
    ...
    def remove2Dstrain(self, fault, computeNormFact=True):
        '''
        ...
        # Computes the strain
        self.compute2Dstrain(fault, computeNormFact=computeNormFact)
    ...
    # ----------------------------------------------------------------------
    def getEulerMatrix(self, components=2):
        '''
        貌似可以自适应三维情况
        '''
        ...
        # Get the number of Euler rotation parameters
        nc = 3

        # Get the longitude and latitude of each stations
        lon, lat = self.lon, self.lat

        # Get the radius of Earth
        r = ERADIUS

        # Allocate a Euler rotation base
        EulerMat = np.zeros((components, nc))
        
        # Allocate the full rotation Matrix
        rotMat = np.zeros((nd, nc))
        ...
        for i in range(ns):
            ...
            EulerMat[:2, :] = np.array([
                [-r*np.cos(lam)*np.sin(phi), -r*np.sin(lam)*np.sin(phi), r*np.cos(phi)],
                [r*np.sin(lam), -r*np.cos(lam), 0]
            ])
            
            rotMat[i, :] = EulerMat[0, :]
            rotMat[i+ns, :] = EulerMat[1, :]

        return rotMat
    # ----------------------------------------------------------------------

    # ----------------------------------------------------------------------
    def genEulerMatrix(self, lonc, latc, components=2):
        '''
        貌似可以自适应三维情况
        '''
    ...
    # ----------------------------------------------------------------------
    def computeEulerRotation(self, fault):
        '''
        '''
        # Compute the transformation
        ...
        # Store the transform here
        self.EulerRot = self.transformation
        self.EulerRotSol = fault.polysol[self.name]
    ...
    # ----------------------------------------------------------------------
    def removeEulerRotation(self, fault):
        '''
        '''
        # Compute the Euler rotation
        self.computeEulerRotation(fault)

        # Correct
        self.vel_enu = self.vel_enu - self.EulerRot
    ...
    # ----------------------------------------------------------------------
    def getInternalStrain(self, components=2, computeIntStrainNormFact=True):
        '''
        可以自适应三维的情况
        '''
        ...
        # Get the number of Internal Strain parameters
        nc = 3
        ...
        # Allocate a Euler rotation base
        IntMat = np.zeros((components, nc))
        
        # Allocate the full rotation Matrix
        StrainMat = np.zeros((nd, nc))

        # Get the position of center of the network
        if computeIntStrainNormFact:
            ...
            # save
            self.InternalStrainNormalizingFactor = {}
            self.InternalStrainNormalizingFactor['ref'] = [lon0, lat0]
            self.InternalStrainNormalizingFactor['base'] = base_max
        else:
            ...
            delta_lon /= base_max
            delta_lat /= base_max
        # self.InternalStrainCenter = [lon0, lat0]
        ...
        for i in range(ns):
            ...
            StrainMat[i+ns, :] = IntMat[1, :]

        return StrainMat
    # ----------------------------------------------------------------------

    # ----------------------------------------------------------------------
    def computeInternalStrain(self, fault, computeIntStrainNormFact=True):
        '''
        '''
        # Compute the transformation
        self.computeTransformation(fault, computeIntStrainNormFact=computeIntStrainNormFact)

        # Store the transform here
        self.IntStrain = self.transformation
        self.IntStrainSol = fault.polysol[self.name]
    ...
    # ----------------------------------------------------------------------
    def removeInternalStrain(self, fault, computeIntStrainNormFact=True):
        '''
        '''
        # Compute the Euler rotation
        ...
        # Correct
        self.vel_enu = self.vel_enu - self.IntStrain
            ...
            if poly == 'build' or poly == 'include':
                if (self.name in fault.poly.keys()):
                    ...
                    if type(gpsref) is str:
                        ...
                        elif gpsref == 'eulerrotation':
                            self.computeEulerRotation(fault)
                            self.synth = self.synth + self.EulerRot
                        elif gpsref == 'internalstrain':
                            self.computeInternalStrain(fault, computeIntStrainNormFact=computeIntStrainNormFact)
    ...
    def initializeTimeSeries(self, start=None, end=None, stationfile=False,
                             ...
                             factor=1., stationfile_dir='./', suffix=''):
        '''
        ...
        Kwargs:
            ...
            * los           : Los vector
            * factor        : scaling factor
            * stationfile_dir: Directory of the station file
            * suffix        : Suffix to be added to the station file name
        ...
        for station in self.station:
            ...
            if (start is not None and end is not None) or (time is not None):
                ...
                filename = os.path.join(stationfile_dir, '{}{}.dat'.format(station, suffix))
    ...
    def writeTimeSeries(self, data_type='value', verbose=False, outdir='./', steplike=False):
        '''
        ...
        for station in self.timeseries:
            ...
            # Write 2 file
            filename = os.path.join(outdir, filename) # Added by kfhe at 4/12/2023
            timeseries.write2file(filename, data_type=data_type, steplike=steplike)
    ...
    def plot_gpstimeseries_at_site(self, sitename, reference_time, direction='EN', timeunit='D', figsize=(7.0, 1.8), 
        ...
        '''
        gpssite = self.timeseries[sitename]

        fig = gpssite.plot_gpstimeseries(reference_time, direction=direction, timeunit=timeunit, 
                                   figsize=figsize, dispunit=dispunit, style=style, fontsize=fontsize,
                                   legend_frame=legend_frame, xlim=xlim)
