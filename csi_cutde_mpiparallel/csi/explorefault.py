...
class explorefault(SourceInv):
    ...
    def walk(self, niter=10000, nburn=5000, method='Metropolis'):
        '''
        March the MCMC.
        ...
        Kwargs:
            ...
            * method            : One of the stepmethods of PyMC3
        ...
        with pymc.Model() as model:
            # Define the prior
            def logp_func(value=self.initSampleVec):
                prob = 0.
            ...
            prior = pymc.DensityDist('prior', logp=logp_func, shape=len(self.initSampleVec))
            ...
            for like in self.Likelihoods:
                ...
                mu = pymc.Deterministic('mu', forward(prior))
                likelihood = pymc.MvNormal('Data Likelihood: {}'.format(data.name), 
                                        mu=mu, 
                                        cov=Cd, 
                                        observed=dobs)

            # Sample
            step = getattr(pymc, method)(model.vars)
            trace = pymc.sample(niter, step=step, tune=nburn, discard_tuned_samples=True)

        # Save the trace
        self.trace = trace
        self.nsamples = len(trace)
