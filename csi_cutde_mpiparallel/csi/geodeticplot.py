...
class geodeticplot(object):
    ...
    def savefig(self, prefix, mapaxis='equal', ftype='pdf', dpi=None, bbox_inches=None, 
                      triDaxis=None, saveFig=['fault', 'map']):
        '''
        Save to file.
        ...
        Kwargs:
            ...
            * triDaxis      : 3D axis scaling # auto None
    ...
    def setzaxis(self, depth, zticklabels=None):
        ...
        if zticklabels is None:
            ...
            for z in zticklabels:
                zticks.append(-1.0*z)
        # Set the ticks and the labels of the z-axis with 2-decimal precision
        zticks = np.round(zticks, decimals=2)
        zticklabels = np.round(zticklabels, decimals=2)
    ...
    def surfacestress(self, stress, component='normal', linewidth=0.0, norm=None, 
        ...
        lat = stress.lat
        #lon[np.logical_or(lon<self.lonmin, lon>self.lonmax)] += 360.
        sc = self.carte.scatter(lon, lat, s=20, c=val, cmap=cmap, vmin=vmin, vmax=vmax, linewidth=linewidth)
