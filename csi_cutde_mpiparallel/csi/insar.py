...
class insar(SourceInv):
    ...
    def checkLosEqualsOne(self):
        # Check
        if self.los is not None:
            uRemove = np.flatnonzero(self.los[:,2]==1.0)
        else:
            uRemove = np.array([])
    ...
    def read_from_ascii_simple(self, filename, factor=1.0, step=0.0, header=0, los=None):
        ...
        for i in range(len(Lines)):
            ...
            line = Lines[i].split()
            # Fill in the values
            self.lon.append(np.float32(line[0]))
            self.lat.append(np.float32(line[1]))
            self.vel.append(np.float32(line[2]))
            if len(line)>3:
                self.err.append(np.float32(line[3]))
            else:
                self.err.append(0.0)
    ...
    def read_from_ascii(self, filename, factor=1.0, step=0.0, header=0):
        ...
        for i in range(header,len(Lines)):
            ...
            self.vel.append(np.float32(line[2]))
            self.err.append(np.float32(line[3]))
            self.los.append([np.float32(line[4]), np.float32(line[5]), np.float32(line[6])])
    ...
    def read_from_varres(self,filename, factor=1.0, step=0.0, header=2, cov=False):
        ...
        for i in range(header, len(A)):
            tmp = A[i].split()
            self.vel.append(np.float32(tmp[5]))
            self.lon.append(np.float32(tmp[3]))
    ...
    def read_from_binary(self, data, lon, lat, err=None, factor=1.0,
        ...
        if heading is not None:
            ...
            elif type(incidence) in (float, np.float_):
    ...
    def extractAroundGPS(self, gps, distance, doprojection=True):
        ...
        for ista, ilon, ilat in zip(out.station, out.lon, out.lat):
        ...
        # Convert to arrays
        out.vel_los = np.array(vel_los)
        out.err_los = np.array(err_los)
        out.los = np.array(los)

        # Do a projection
        if doprojection and out.lon.shape[0] == gps.lon.shape[0]:
    ...
    def keepPixels(self, u):
        ...
        if self.corner is not None and len(self.corner) != 0:
    ...
    def setGFsInFault(self, fault, G, vertical=True):
        ...
        if fault.type == "Fault":
            ...
            fault.setGFs(self, strikeslip=[GFs['strikeslip']], dipslip=[GFs['dipslip']], 
                        tensile=[GFs['tensile']], coupling=[GFs['coupling']], vertical=True)
        elif fault.type == "Pressure":
            keys = ['pressure', 'pressureDVx', 'pressureDVy', 'pressureDVz']
            GFs = {key: G.get(key, None) for key in keys}
            fault.setGFs(self, deltapressure=[GFs['pressure']], GDVx=[GFs['pressureDVx']], 
                        GDVy=[GFs['pressureDVy']], GDVz=[GFs['pressureDVz']], vertical=True)
    ...
    def getEulerMatrix(self):
        ...
        Hre[:, 0] = -r*np.cos(lam)*np.sin(phi)
        Hre[:, 1] = -r*np.sin(lam)*np.sin(phi)
        Hre[:, 2] = r*np.cos(phi)
        Hrn[:, 0] = r*np.sin(lam)
        Hrn[:, 1] = -r*np.cos(lam)

        # Multiply by the los 
        rotMat = self.los[:, 0][:, np.newaxis]*Hre + self.los[:, 1][:, np.newaxis]*Hrn
    ...
    # ----------------------------------------------------------------------
    def genEulerMatrix(self, lonc, latc):
        '''
        貌似可以自适应三维情况
        '''
    ...
    def removeEulerRotation(self, fault):
        ...
        # Correct
        self.vel = self.vel - self.EulerRot
    ...
    # ----------------------------------------------------------------------
    def getInternalStrain(self, computeIntStrainNormFact=True):
        '''
        可以自适应三维的情况
        '''
    ...
    def removeInternalStrain(self, fault, computeIntStrainNormFact=True):
        ...
        # Correct
        self.vel = self.vel - self.IntStrain
    ...
    def getNumberOfTransformParameters(self, transformation):
        ...
        if transformation.__class__ in (str, int):
    ...
    def getTransformEstimator(self, transformation, computeNormFact=True, computeIntStrainNormFact=True):
        ...
        orb = np.empty((self.vel.shape[0], 0))
        for itransformation in transformation:
            # Strain + Rotation + Translation
            if itransformation == 'strain':
                tmporb = self.get2DstrainEst(computeNormFact=computeNormFact)
            elif itransformation == 'internalstrain':
                tmporb = self.getInternalStrain(computeIntStrainNormFact=computeIntStrainNormFact)
            elif itransformation == 'eulerrotation':
    ...
    # ----------------------------------------------------------------------
    def compute2Dstrain(self, fault, computeNormFact=True, write2file=False, verbose=False):
        '''
    ...
    def removeSynth(self, faults, direction='sd', poly=None, vertical=True, custom=False, computeNormFact=True, computeIntStrainNormFact=True):
        '''
        ...
        # Build synthetics
        self.buildsynth(faults, direction=direction, poly=poly, custom=custom, computeNormFact=computeNormFact, computeIntStrainNormFact=computeIntStrainNormFact)
    ...
    def computeTransformation(self, fault, verbose=False, custom=False, computeNormFact=True, computeIntStrainNormFact=True):
        ...
        # make the array
        self.transformation = np.zeros(self.vel.shape)
        ...
        for itransformation in transformation:
            ...
            # Fill it
            self.transformation += tmpsynth

        # Compute custom
        if custom:
            self.computeCustom(fault)
            self.transformation += self.custompred
            ...
            if fault.type == "Fault":
                ...
                if poly == 'build' or poly == 'include':
                    if (self.name in fault.poly.keys()):
                        sarref = fault.poly[self.name]
                        if type(sarref) is str:
                            if sarref in ('strain',):
                                self.compute2Dstrain(fault, computeNormFact=computeNormFact)
                                self.synth = self.synth + self.Strain
                            elif sarref == 'eulerrotation':
                                self.computeEulerRotation(fault)
                                self.synth = self.synth + self.EulerRot
                            elif sarref == 'internalstrain':
                                self.computeInternalStrain(fault, computeIntStrainNormFact=computeIntStrainNormFact)
