...
class insartimeseries(insar):
    ...
    def read_from_sarfiles(self, obsdates, dirname, sarfile_pattern, factor=1.0, factor_obj='both'):
        '''
        ...
        for tsdate in obsdates:
            sarfile = os.path.join(dirname, sarfile_pattern.format(tsdate))
            sarname = tsdate.isoformat()
            sar = insar(sarname, utmzone=self.utmzone)
            sar.read_from_varres(sarfile, cov=True)
            if factor_obj == 'vel':
                sar.vel *= factor
                sar.factor_vel = factor
            elif factor_obj == 'err':
                sar.err *= factor
                sar.factor_err = factor
            else:
                sar.vel *= factor
                sar.err *= factor
                sar.factor = factor
            self.time.append(tsdate)
            self.timeseries.append(sar)
            self.setLonLat(sar.lon, sar.lat)
    ...
    def extractAroundGPS(self, gps, distance, doprojection=True, reference=False, verbose=False):
        ...
        # Iterate over time
        for idate,isar in enumerate(self.timeseries):
            if verbose:
                sys.stdout.write('\r Date: {}'.format(self.time[idate].isoformat()))
            # Extract the values at this date
            tmp = isar.extractAroundGPS(gps, distance, doprojection=False) # gps
        ...
        if reference:
            for station in gps.station:
                ...
                isar = out.timeseries[station].los
                gpspr = gps.timeseries[station].los
                if isar is not None and gpspr is not None:
                    # Find the common dates 
                    diff = []
                    for itime, time in enumerate(isar.time):
                        u = np.flatnonzero(gpspr.time==time)
                        if len(u)>0:
                            diff.append(isar.value[itime]-gpspr.value[u[0]])
                    # Average and correct
                    if len(diff)>0:
