...
class multigps(gps):
    ...
    def getNumberOfTransformParameters(self, transformation):
        ...
        nMain = super(multigps,self).getNumberOfTransformParameters([transformation[0]]) # changed by kfhe at 01/15/2023
    ...
    def getTransformEstimator(self, transformation, computeNormFact=True, computeIntStrainNormFact=True):
        ...
        Morb = super(multigps,self).getTransformEstimator(mainTrans, 
                                                          computeNormFact=computeNormFact, computeIntStrainNormFact=computeIntStrainNormFact)
        ...
        for trans, gp in zip(subTrans, self.gpsobjects):
            # Get the transform
            Sorb = gp.getTransformEstimator(trans, computeNormFact=computeNormFact, computeIntStrainNormFact=computeIntStrainNormFact)
            if Sorb is not None:
    ...
    def computeTransformation(self, fault, custom=False, computeNormFact=True, computeIntStrainNormFact=True):
        '''
        ...
        # Get the solution
        Tvec = fault.polysol[self.name] # ['{}'.format(transformation)] # commentted by kfhe at 01/14/2023
        ...
        # Get the estimator
        orb = self.getTransformEstimator(transformation, computeNormFact=computeNormFact, computeIntStrainNormFact=computeIntStrainNormFact)
