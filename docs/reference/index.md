...
| [反演数据改正项](data_corrections.md) | 需要确认反演中的 `geodata.polys`、InSAR ramp、GPS frame transform、`poly_bounds` 或 `data_corrections` 语义时 |
...
| [降采样前观测改正与网格导出](observation_correction_export.md) | 需要在降采样前归零/去一阶平面、给已确认的不连通相位分量修正整周，或无重采样导出全分辨率 SAR/optical 网格时 |
