...
三角网格发生非刚性更新后，旧面积缓存会被标记为空，避免继续使用过期面积。summary 只在需要
报告面积时调用 fault 的 `get_patch_areas()`：有效缓存直接复用，缓存失效时按当前几何计算。
它不会使用旧值、伪造默认面积，也不会要求用户为汇总另加配置。
对于本身没有面积概念的其他 source，单源面积保持 `None`，多源总面积只累加实际存在的值，
不会因此中断其余断层的统计。
