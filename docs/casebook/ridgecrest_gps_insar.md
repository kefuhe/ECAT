...
`read_from_ascii` 的列语义是 `lon lat data err Elos Nlos Ulos`。这里 `Los(m)` 是 LOS 观测，`E/N/U` 是 LOS 投影向量；第 4 列会被读成 `sar.err` 并用于 `buildDiagCd()`，所以文档需要交代 `wt` 在这个案例里按误差列处理。更完整的数据格式说明见 [InSAR 与 GPS 数据读取](../workflows/01_data_reading_insar_gps.md)。
...
这一段要和 [InSAR 与 GPS 数据读取](../workflows/01_data_reading_insar_gps.md#insar-数据入口) 对应起来讲，特别是 ASCII 第 4 列按 `sar.err` 读入这一点。
