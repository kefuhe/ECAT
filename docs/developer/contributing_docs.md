# 文档维护规范

用户手册的目标是帮助科研用户完成可重复工作流，而不是展示所有实现细节。

## 基本规则

- 用户手册放在 [ECAT 仓库 `docs/`](https://github.com/kefuhe/ECAT/tree/main/docs)。
- 不在公开手册中引用本地绝对路径、未整理笔记或未随手册发布的材料。
- 优先写经过整理的工作流，不直接堆脚本。
- 每个案例都要有可复现命令、输入说明和输出说明。
- 每个案例的教学重点都要落到脚本或配置片段上，说明“这段代码做什么、为什么要这样写、下一步接到哪里”。
- 大型 HDF5 样本和图件应放在 [ECAT-Cases](https://github.com/kefuhe/ECAT-Cases)，不要放进 [ECAT](https://github.com/kefuhe/ECAT) 主代码仓库。
- 导航入口使用“中文 / English (`directory/`)”格式，例如“工作流 / Workflows (`workflows/`)”，让用户能直接对应 GitHub 目录。
- 跨页链接如果跳到小节，优先使用稳定英文锚点；中文小节被外部引用时，在标题前加 `<a id="english-anchor"></a>`。
- 公开用户路线以顶层 `docs/` 为唯一入口；设计草稿、调试记录、任务计划和 changelog
  不能原样搬进用户手册，应先按用户任务重写。
- eqtools 的 workflow、example 和 reference 可以从独立开发仓库选择性同步；
  `getting_started/installation.md`、顶层导航和发布边界属于 ECAT 集成层，不能用
  包级 README 或机器环境说明整页覆盖。
- `examples/` 只放短小任务例子，使用通用文件名和占位路径；完整真实事件脚本仍放在 `casebook/` 和 ECAT-Cases。
- `concepts/` 只解释跨页面复用的概念和对象关系，不放完整 API 字段表。
- 完整脚本模板不等于公开案例；只有 ECAT-Cases 中已有可访问数据入口、命令和预期输出时，
  才把内容登记到 `casebook/`。
- 文档中的“检查”“无界面”“不绘图”“续跑”和“覆盖”必须与真实入口一致；不要把
  配置预检写成脱离数据的语法检查，也不要把 `MPLBACKEND=Agg` 写成关闭绘图。

## 案例页面模板

```markdown
# 事件名：工作流类型

## 位置

## 为什么选这个案例

## 数据入口

## 运行方式

## 脚本对照

### 1. 第一步对应的代码

### 2. 第二步对应的配置

## 应保留输出

## 已知限制
```

## 短例页面模板

```markdown
# 任务名

一句话说明这个例子解决什么小任务。

## 输入

需要准备的最少文件、对象或配置。

## 最小代码

可复制代码或命令。文件名使用通用占位符，不写本地绝对路径。

## 检查

运行后应看到哪些输出，如何判断方向、单位、维度或数量是否合理。

## 何时不用这个例子

列出 2-3 个应转去 workflow、casebook 或 reference 的情况。

## 相关参考

链接到对应 workflow、reference 和案例。
```

## 概念页面模板

```markdown
# 概念名

一句话说明这个概念解决什么混淆。

## 核心判断

说明概念之间的关系、推荐路线或常见误区。

## 继续阅读

链接到 workflow、examples、reference 或 casebook。
```

## 整理技术材料时

从实现笔记、调试记录或案例脚本整理到公开手册时，要按用户学习路线重写：

- 从用户动作开始。
- 先给最小可运行命令。
- 当前阶段只解释必要参数。
- 复杂选项链接到 reference 页。
- 避免把未经整理的实验脚本直接写成教程。

## Reference 页面模板

`reference/` 页面可以很完整，但开头应保持轻量、可导航：

```markdown
# 页面标题

一句话说明本页负责什么，并链接到对应 workflow 或 casebook。

## 阅读路径

- 第一次运行应先看哪一页。
- 当前页中最常查的 2-4 个小节。
- 高级或维护场景应跳到哪里。

## 主体参考内容

字段、API、配置块、输出、诊断、常见误区和相关页面。
```

Reference 内容不要因为页面长而删除字段字典；应通过分组、阅读路径和相关页面链接降低首次阅读负担。

## 代码修改后的文档影响评估

完整判断与跨包所有权见[代码—文档影响评估](documentation_impact.md)。每次代码修改都应
先判断公开行为是否变化，而不是根据文件名机械修改页面。从仓库根目录运行：

```bash
python maintainer_tools/check_docs_impact.py --repo eqtools
```

输出列出必须重新核对的公开页面和聚焦测试。它是候选路由，不代表所有列出页面都必须改动。
判断规则如下：

- 公开 API、CLI、YAML、输出、科学约定、工作流或用户可见错误变化时，更新公开页面；
- 只有内部状态、缓存或调用链变化而用户接口不变时，不向公开手册复制实现审计；
- 只改内部变量且上述语义均不变时，可以不改文档，但任务结果必须报告
  `Docs impact: none` 及理由；
- 示例必须对照真实 parser、签名、registry、配置模板或测试，不能只复制旧脚本。

跨 eqtools/CSI 的底层变化应同时检查上层公开页面，但用户手册只解释稳定接口、科学约定和
用户可见行为，不复制两个源码包之间的内部调用链。
