...
这段旧脚本只用于理解历史映射。当前 ECAT CLI 写矩形 `.rsp` 时默认保存 18 列 full-corner cell，检查图按四个真实角点绘制；若直接调用 CSI Python API 复现当前矩形输出，应使用支持 full-corner 的写法，而不是只依赖旧的 10 列中心点/宽高 `.rsp`。
